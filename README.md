...
|   |   |-- participant_ready.py             # Filter the classified dataset down to participant-ready rows
...
- `build-participant-ready-dataset`: filter the classified dataset down to participant-ready rows
...
Primary kit source file: `data/datasets/classified/dataset_participant_ready.csv`

Upstream classified source file: `data/datasets/classified/dataset_classified.csv`
...
Behavior:
- raw classified rows are filtered to participant-ready rows before kit sampling
- rows with `NVD-CWE-NOINFO`, `Unknown/Unspecified`, or obvious malformed one-line comment flattening are excluded from participant kits
...
- participant-ready dataset used for kit sampling: `data/datasets/classified/dataset_participant_ready.csv`
- rejected-row audit file: `data/datasets/classified/dataset_participant_rejected.csv`
...
```
4. Build the participant-ready dataset:
```powershell
...
```
5. Use the participant-ready CSV as the kit source in the GUI or CLI.
...
|-- README.md
|-- exports/                                # Created after Finish (Build ZIP)
`-- .repairaudit/                           # Hidden support folder; participants should leave it alone
...
Notes:
- Participants should only need the launcher and the README at the top level.
- On Windows, `.repairaudit` is hidden with the folder hidden attribute. On macOS/Linux it remains dot-prefixed.
